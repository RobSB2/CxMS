 ...
 * Telemetry is ENABLED BY DEFAULT (opt-out model) to help improve CxMS.
 *
 * Usage:
 *   node cxms-report.mjs              # Default mode (prompts for feedback)
 *   node cxms-report.mjs --dry-run    # Preview without sending
 *   node cxms-report.mjs --full       # Comprehensive survey
 *   node cxms-report.mjs --quiet      # Minimal output (for automation)
 *   node cxms-report.mjs --help       # Show help
 *
 * Consent Management (opt-out):
 *   node cxms-report.mjs --revoke           # Opt out of telemetry
 *   node cxms-report.mjs --consent          # Explicitly opt back in
 *   node cxms-report.mjs --status           # Check current status
 *
 * Session Integration:
 *   node cxms-report.mjs --auto             # Auto-submit at session end
 *   node cxms-report.mjs --auto --quiet     # Silent auto-submit
 *   node cxms-report.mjs --retry-queue      # Retry any failed submissions
 *
 * Version: 1.2.0
 */
...
function getConsent(cwd) {
  ...
  // OPT-OUT MODEL: No consent file = enabled by default
  return { consented: true, auto_submit: true, implicit: true };
}

function isExplicitlyRevoked(cwd) {
  const consent = getConsent(cwd);
  // Only revoked if explicit consent file exists with consented: false
  const consentFile = getConsentFilePath(cwd);
  if (!fs.existsSync(consentFile)) return false;
  return consent && consent.consented === false;
}

function hasSeenOptOutNotice(cwd) {
  const noticeFile = path.join(cwd, '.cxms', '.telemetry-notice-shown');
  return fs.existsSync(noticeFile);
}

function markOptOutNoticeSeen(cwd) {
  const cxmsDir = path.join(cwd, '.cxms');
  if (!fs.existsSync(cxmsDir)) {
    fs.mkdirSync(cxmsDir, { recursive: true });
  }
  fs.writeFileSync(path.join(cxmsDir, '.telemetry-notice-shown'), new Date().toISOString());
}
...
function displayConsentStatus(cwd, quiet = false) {
  const consent = getConsent(cwd);
  const consentFile = getConsentFilePath(cwd);
  const hasExplicitFile = fs.existsSync(consentFile);

  if (quiet) {
    if (isExplicitlyRevoked(cwd)) {
      log('Telemetry: DISABLED (opted out)');
    } else {
      log('Telemetry: ENABLED' + (consent.implicit ? ' (default)' : ''));
    }
    return consent;
  }

  log('\n════════════════════════════════════════════════════════════');
  log('    CxMS Telemetry Status');
  log('════════════════════════════════════════════════════════════\n');

  if (isExplicitlyRevoked(cwd)) {
    log('   Status:        DISABLED (opted out)');
    log('   Auto-submit:   Disabled');
    log('\n   Run with --consent to re-enable telemetry.\n');
    return consent;
  }

  const status = hasExplicitFile ? 'ENABLED (explicit)' : 'ENABLED (default)';
  log(`   Status:        ${status}`);
  log(`   Auto-submit:   ${consent.auto_submit ? 'Yes' : 'No'}`);
  if (consent.consent_date) {
    log(`   Since:         ${consent.consent_date}`);
  }
  if (consent.last_submission) {
    log(`   Last Submit:   ${consent.last_submission}`);
  }
  log('\n   Run with --revoke to opt out of telemetry.\n');

  return consent;
}

// ============================================
// QUEUE MANAGEMENT (for resilient submission)
// ============================================

function getQueueFilePath(cwd) {
  return path.join(cwd, '.cxms', 'telemetry-queue.json');
}

function queueSubmission(cwd, data) {
  const cxmsDir = path.join(cwd, '.cxms');
  if (!fs.existsSync(cxmsDir)) {
    fs.mkdirSync(cxmsDir, { recursive: true });
  }

  const queueFile = getQueueFilePath(cwd);
  let queue = [];
  ...
  // Add timestamp and queue the data
  data.queued_at = new Date().toISOString();
  queue.push(data);

  // Keep only last 5 queued submissions to avoid unbounded growth
  if (queue.length > 5) {
    queue = queue.slice(-5);
  }
...
function getQueuedSubmissions(cwd) {
  const queueFile = getQueueFilePath(cwd);
...
function removeFromQueue(cwd, index) {
  const queueFile = getQueueFilePath(cwd);
  const queue = getQueuedSubmissions(cwd);
...
async function main() {
  ...
  const retryQueueMode = args.includes('--retry-queue');
  ...
  const qlog = (msg) => { if (!quietMode) log(msg); };

  if (helpMode) {
    log(`
CxMS Telemetry Reporter v${CLIENT_VERSION}

Telemetry is ENABLED BY DEFAULT to help improve CxMS.
No personal info, project names, or code is collected.

Usage:
  node cxms-report.mjs              Interactive mode (metrics + feedback)
  node cxms-report.mjs --full       Comprehensive survey
  node cxms-report.mjs --dry-run    Preview data without sending
  node cxms-report.mjs --quiet      Minimal output (for automation)
  node cxms-report.mjs --help       Show this help

Opt-Out:
  node cxms-report.mjs --revoke     Disable telemetry (opt out)
  node cxms-report.mjs --consent    Re-enable telemetry (opt back in)
  node cxms-report.mjs --status     Check current status
...
The --auto flag queues data locally first, then attempts remote submit.
If the session ends before submission completes, data is preserved and
retried on next session start with --retry-queue.
...
Learn more: https://github.com/RobSB2/CxMS
  ...
  if (statusMode) {
    displayConsentStatus(cwd, quietMode);
    process.exit(0);
  }

  if (consentMode) {
    saveConsent(cwd, true, true);
    if (!quietMode) {
      log('\n✅ Telemetry enabled.');
      log('   Dashboard: https://robsb2.github.io/CxMS/dashboard');
      log('   Run --revoke to opt out.\n');
    }
    process.exit(0);
  }
  ...
  if (revokeMode) {
    ...
    if (!quietMode) {
      log('\n✅ Telemetry disabled (opted out).');
      log('   Run --consent to re-enable.\n');
    }
    process.exit(0);
  }

  // Retry queue: process any failed submissions from previous sessions
  if (retryQueueMode) {
    const queue = getQueuedSubmissions(cwd);
    if (queue.length === 0) {
      if (!quietMode) log('No queued submissions.');
      process.exit(0);
    }

    if (isExplicitlyRevoked(cwd)) {
      clearQueue(cwd);
      if (!quietMode) log('Telemetry disabled. Cleared queued submissions.');
      process.exit(0);
    }

    qlog(`\n🔄 Retrying ${queue.length} queued submission(s)...`);
    ...
    for (let i = queue.length - 1; i >= 0; i--) {
      try {
        // Update submission timestamp for retry
        queue[i].submitted_at = new Date().toISOString();
        queue[i].was_queued = true;
        await submitTelemetry(queue[i]);
        removeFromQueue(cwd, i);
        success++;
      } catch (err) {
        failed++;
        if (!quietMode) log(`   Failed: ${err.message}`);
      }
    }

    if (quietMode) {
      if (success > 0) log(`Telemetry: ${success} queued submission(s) sent`);
    } else {
      if (success > 0) log(`✅ ${success} submission(s) sent`);
      if (failed > 0) log(`⚠️  ${failed} submission(s) still queued (will retry next time)`);
    }
    process.exit(0);
  }

  // Auto mode: check if opted out
  if (autoMode) {
    if (isExplicitlyRevoked(cwd)) {
      if (!quietMode) log('Telemetry disabled (opted out). Skipping.');
      process.exit(0);
    }

    // Show one-time opt-out notice if this is first auto-run
    if (!hasSeenOptOutNotice(cwd) && !quietMode) {
      log('\n╔══════════════════════════════════════════════════════════════╗');
      log('║  CxMS Telemetry is enabled by default to improve the system. ║');
      log('║  No personal info, project names, or code is collected.      ║');
      log('║  To opt out: node tools/cxms-report.mjs --revoke             ║');
      log('║  Dashboard: https://robsb2.github.io/CxMS/dashboard          ║');
      log('╚══════════════════════════════════════════════════════════════╝\n');
      markOptOutNoticeSeen(cwd);
    }

    qlog('🔄 Submitting telemetry...');
  }

  qlog('\n════════════════════════════════════════════════════════════');
  qlog(`    CxMS Telemetry Reporter v${CLIENT_VERSION}`);
  qlog('════════════════════════════════════════════════════════════');
  ...
  qlog(`   Found ${fileCount} CxMS files`);

  // Extract metrics
  qlog('\n📊 Extracting metrics...');
  let data = extractAllMetrics(cwd, files);

  // Collect user feedback (skip if --skip-prompts, --yes, --auto, or --quiet)
  if (!skipPrompts && !autoYes && !autoMode && !quietMode) {
  ...
  // Display summary (skip in quiet mode)
  if (!quietMode) {
    displaySummary(data);
  }
  ...
  if (!autoYes && !autoMode && !quietMode) {
    ...
    log('║  No personal info, project names, or code is collected.       ║');
    log('╚══════════════════════════════════════════════════════════════╝\n');
    userConsent = await ask('Submit this anonymous report? [y/N]: ');
  } else if (autoMode && !quietMode) {
    qlog('Auto-submitting...');
  } else if (autoYes && !quietMode) {
    log('\n✅ Auto-consenting (--yes flag)');
  }

  if (!autoMode && !autoYes && userConsent.toLowerCase() !== 'y') {
  ...
  // For auto mode: queue first for resilience, then attempt submit
  if (autoMode) {
    // Queue immediately (survives if session ends)
    queueSubmission(cwd, { ...data });

    // Now attempt actual submission
    try {
      ...
      if (quietMode) {
        log('Telemetry: submitted');
      } else {
        log('✅ Telemetry submitted.');
      }
    } catch (err) {
      // Failed - data is queued, will retry next session
      if (quietMode) {
        log('Telemetry: queued (will retry)');
      } else {
        log(`⚠️  Submit failed (${err.message}). Data queued for retry.`);
      }
    }
    process.exit(0);
  }

  // Interactive/--yes mode: direct submission
  qlog('\n📤 Submitting...');
  qlog(`   (Script took ${data.timing.script_duration_seconds}s)`);
  ...
  try {
    ...
    qlog('   View community dashboard: https://robsb2.github.io/CxMS/dashboard\n');
  } catch (err) {
